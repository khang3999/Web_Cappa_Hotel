import Navbar from "@/Components/Commons/Navbar";
import { Head } from "@inertiajs/react";
import '../../../css/roomDetail.css'
import '../../../css/payment.css'
import Footer from "@/Components/Commons/Footer";

const Payment = ({ roomId, checkin, checkout, price }) => {
    return (
        <>
            <Head title="Cappa Luxury Hotel" />
            {/* <div className="header img-base position-relative">
                <Navbar></Navbar>
                <img src="/images/contact-slide-1.jpg" alt="" className="banner w-100 img-fluid" />
            </div> */}
            <h4 className="text-center m-5 ">YOUR BOOKING</h4>
            <div className="row m-5 text-center border border-warning">
                <div className="col-3 border border-warning ">Room</div>
                <div className="col-3 border border-warning ">Checkin</div>
                <div className="col-3 border border-warning ">Checkout</div>
                <div className="col-3 border border-warning ">Price</div>
                <div className="col-3 border border-warning">{roomId}</div>
                <div className="col-3 border border-warning">{checkin}</div>
                <div className="col-3 border border-warning">{checkout}</div>
                <div className="col-3 border border-warning">{price}</div>
            </div>
            <h1 className="text-center">Please check the QR code to make payment</h1>
            <div class="flip-card d-flex justify-content-center mb-5 mx-auto">
                <div class="flip-card-inner">
                    <div class="flip-card-front">
                        <p class="heading_8264">MASTERCARD</p>
                        <svg class="logo" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="36" height="36" viewBox="0 0 48 48">
                            <path fill="#ff9800" d="M32 10A14 14 0 1 0 32 38A14 14 0 1 0 32 10Z"></path><path fill="#d50000" d="M16 10A14 14 0 1 0 16 38A14 14 0 1 0 16 10Z"></path><path fill="#ff3d00" d="M18,24c0,4.755,2.376,8.95,6,11.48c3.624-2.53,6-6.725,6-11.48s-2.376-8.95-6-11.48 C20.376,15.05,18,19.245,18,24z"></path>
                        </svg>
                        <svg version="1.1" class="chip" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="30px" height="30px" viewBox="0 0 50 50" xml:space="preserve">  <image id="image0" width="50" height="50" x="0" y="0" href="data:image/png;base64,"></image>
                        </svg>
                        <svg version="1.1" class="contactless" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="20px" height="20px" viewBox="0 0 50 50" xml:space="preserve">  <image id="image0" width="50" height="50" x="0" y="0" href="data:image/png;base64,"></image>
                        </svg>
                        <p class="number">0252 0253 4701 9999</p>
                        <p class="valid_thru">VALID THRU</p>
                        <p class="date_8264">1 2 / 2 4</p>
                        <p class="name">Cappa Luxury Hotel</p>
                    </div>
                    <div class="flip-card-back d-flex justify-content-center ">
                        <img src="/images/QR.png" alt="" className="img img-fluid qr mx-auto" />
                    </div>
                </div>

            </div>

            <div className=" m-5 fst-italic text-secondary">Note: Transfer content is your Email, admin will check payment and respond via email to you</div>

            <div className="d-flex justify-content-center mb-5">
                <a href={route('indexUser')} className="button">
                    Go home
                </a>
            </div>


            {/* Footer */}
            <Footer></Footer>
        </>
    )
}
export default Payment;